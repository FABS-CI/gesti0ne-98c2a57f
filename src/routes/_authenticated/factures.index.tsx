    ...
    exportCsv(
      ...
      ]),
      {
        ...
        summary: [
          ...
          { label: "Total facturé", value: __FMT__(totals.total) },
          { label: "Total encaissé", value: __FMT__(totals.paye) },
          { label: "Reste à recouvrer", value: __FMT__(totals.du) },
        ],
      },
    );
  }
  ...
  function handleExportPDF() {
    ...
    exportListePDF({
      ...
      lignes: factures.map((f) => [
        ...
        f.date_facture,
        f.client_nom ?? "",
        __FMTN__(Number(f.montant_total)),
        __FMTN__(Number(f.montant_paye)),
        __FMTN__((Number(f.montant_total) - Number(f.montant_paye))),
        STATUT_FACTURE_LABEL[f.statut]?.label ?? f.statut,
      ]),
      filtres,
      filename: "factures",
