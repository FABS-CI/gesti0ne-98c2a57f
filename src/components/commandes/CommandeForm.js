...
const ligneSchema = z.object({
    ...
    quantite: z.number().int().min(1, "Quantité ≥ 1").optional().or(z.literal(undefined)),
    prix_unitaire: z.number().min(0, "Prix ≥ 0").optional().or(z.literal(undefined)),
...
export function CommandeForm({ mode, commandeId, initialValues, presetClientId }) {
    ...
    const mutation = useMutation({
        mutationFn: (values) => {
            ...
            const payload = {
                ...
                client_nom: values.client_nom || selectedClient?.nom || values.etablissement || null,
            ...
            if (mode === "edit" && commandeId)
                return modifierCommande(commandeId, payload);
        ...
        onSuccess: async (created, submittedValues) => {
            ...
            invalidateCommande(qc, {
                commandeId: commandeId ?? created?.commande_id,
                clientId,
            });
            if (mode === "edit") {
            ...
            setRecap({
                commandeId: cId,
                commandeRef: cRef,
                factureRef: fac?.reference ?? null,
                factureId: fac?.facture_id ?? null,
                blRef: bl?.reference ?? null,
                blId: bl?.bl_id ?? null,
                autoValidated: true,
            });
        },
    ...
    const confirmSubmit = (validate = false) => {
        if (!pendingValues)
            return;
        ...
        queries: lignesWatch.map((l) => ({
            ...
            queryFn: async () => {
                if (!l.produit_id)
                    return null;
                if (depotId)
                    return getStockProduitDepot(l.produit_id, depotId);
    ...
    const overshootIndexes = useMemo(() => {
        ...
        lignesWatch.forEach((l, i) => {
            ...
            if (typeof s === "number" && (l.quantite || 0) > s)
                out.push(i);
        });
        return out;
    ...
    return (<form onSubmit={onSubmit} className="space-y-6 pb-32 2xl:pb-6">
      {mode === "create" && draft.pendingDraft ? (<DraftRestoreBanner label="commande" updatedAt={draft.pendingDraft.updatedAt} onDiscard={() => void draft.discard()} onRestore={() => {
      ...
      <div className="grid gap-6 2xl:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <div className="space-y-6 min-w-0">
          ...
          <section className="relative overflow-hidden rounded-md border bg-card p-4 pl-5 sm:p-5 sm:pl-6 space-y-4">
            ...
            <div>
              ...
              <ClientSearchSelect value={clientId} onChange={(id, client) => {
            ...
            {selectedClient && (<div className="rounded-md bg-muted/50 p-3 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                <InfoCell label="Type" value={selectedClient.type_client}/>
                <InfoCell label="Commercial" value={selectedClient.representant ?? "—"}/>
                <InfoCell label="Plafond crédit" value={formatFCFA(selectedClient.plafond_credit ?? 0)}/>
                <InfoCell label="Solde" value={formatFCFA(selectedClient.solde ?? 0)} emphasis={(selectedClient.solde ?? 0) > (selectedClient.plafond_credit ?? 0)}/>
              </div>)}
          ...
          <section className="relative overflow-hidden rounded-md border bg-card p-4 pl-5 sm:p-5 sm:pl-6 space-y-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <div>
                <DepotSortieField value={form.watch("depot_id") ?? ""} onChange={(id, motif) => {
          ...
          {/* 3. Produits */}
          <LignesSection form={form} fields={fields} lignesWatch={lignesWatch} stockQueries={stockQueries} overshootIndexes={overshootIndexes} totalArticles={totalArticles} totalQuantite={totalQuantite} addLigne={addLigne} remove={remove} onProduitChange={onProduitChange} remiseEnLigneDisabled={hasRemiseGlobale}/>
          ...
          <section className="relative overflow-hidden rounded-md border bg-card p-4 pl-5 sm:p-5 sm:pl-6 space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <NumberField control={form.control} name="remise_globale_pct" step="0.01" min={0} max={100} disabled={hasRemiseEnLigne}/>
                {form.formState.errors.remise_globale_pct && (<p className="text-[10px] text-destructive mt-0.5">{form.formState.errors.remise_globale_pct.message}</p>)}
        ...
        <aside className="hidden 2xl:block">
          <div className="sticky top-6 space-y-4">
            <SummaryCard totaux={totaux} tauxTva={tauxTva} remiseGlobalePct={remiseGlobalePct || 0} totalArticles={totalArticles} totalQuantite={totalQuantite}/>
            <div className="flex flex-col gap-2">
              <Button type="submit" disabled={mutation.isPending || hasOvershoot} className="w-full">
      ...
      <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <AlertDialogContent className="max-w-lg">
          ...
          {pendingValues && (<div className="space-y-3 text-sm max-h-[60vh] overflow-y-auto">
              <div className="grid grid-cols-2 gap-2">
                <InfoCell label="Date" value={pendingValues.date_commande}/>
                <InfoCell label="Client" value={selectedClient?.nom ?? pendingValues.etablissement ?? "—"}/>
              ...
              <div className="grid grid-cols-2 gap-2 rounded-md bg-muted/50 p-3">
                ...
                <InfoCell label={`TVA (${tauxTva}%)`} value={formatFCFA(totaux.tva)}/>
                <InfoCell label={`Remise globale (${pendingValues.remise_globale_pct}%)`} value={`- ${formatFCFA(totaux.remiseGlobaleMontant)}`}/>
        ...
        <AlertDialogContent className="max-w-lg">
          ...
          {recap && (<div className="space-y-2 text-sm">
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                ...
                {recap.factureId && (<Button asChild variant="outline" size="sm">
                    <Link to="/factures">Voir les factures</Link>
                  </Button>)}
                {recap.blId && (<Button asChild variant="outline" size="sm">
      ...
      <AlertDialog open={immediateConfirmOpen} onOpenChange={setImmediateConfirmOpen}>
        <AlertDialogContent className="max-w-md">
          ...
          <div className="grid gap-3 py-4">
            <Button variant="outline" className="justify-start h-auto py-3 px-4 flex-col items-start gap-1" onClick={() => {
            ...
            <Button className="justify-start h-auto py-3 px-4 flex-col items-start gap-1" onClick={() => {
            setImmediateConfirmOpen(false);
            confirmSubmit(true);
        }}>
