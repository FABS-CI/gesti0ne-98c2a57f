...
export async function generateUnifiedCommercialPDF(
  type: "Facture" | "Proforma" | "Commande" | "Bon de Livraison" | "Avoir" | "Spécimens",
  data: DataBase
): Promise<Blob> {
  const docBase = {
    id: (data as any).id || (data as any).facture_id || (data as any).commande_id || (data as any).proforma_id || (data as any).bl_id || "verification-only",
    type: type,
  ...
  const totals = {
    ...
    montantLettres: (data as any).montantLettres || "Arrêtée à la somme de zéro francs CFA",
  };
  ...
  (doc.data as any).soldeDu = data.soldeDu || 0;
