...
export class BaseDocument {
    ...
    drawHeader() {
        ...
        if (!isStatement) {
            const refText = this.data.reference.includes('-') ||
    ...
    drawTable(y, colonnes, lignes, options) {
        // Header
        this.page.drawRectangle({
            x: MARGINS.x,
            y: y - 20,
    ...
    drawTotals(y) {
        ...
        const drawTotalBoxBorders = (height) => {
        ...
        if (this.totals.tva)
            totalRows++;
        if (this.totals.frais)
            totalRows++;
        totalRows++; // Total à payer
        drawTotalBoxBorders(totalRows * 20);
        const row = (label, value, isTotal = false, isNet = false) => {
            if (isTotal) {
