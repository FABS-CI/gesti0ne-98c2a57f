CREATE OR REPLACE FUNCTION public.finaliser_tournee_interne(_tournee_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF NOT public.has_permission(auth.uid(), 'tournees.valider')
     AND NOT public.has_permission(auth.uid(), 'tournees.cloturer') THEN
    RAISE EXCEPTION 'Accès refusé : permission de validation ou clôture requise'
      USING ERRCODE = '42501';
  END IF;
  ...
  IF v_statut = 'annulee' THEN RAISE EXCEPTION 'Une tournée annulée ne peut pas être validée'; END IF;

  SELECT count(*) INTO v_colis FROM public.colis WHERE tournee_id = _tournee_id;
  IF v_colis = 0 THEN
    RAISE EXCEPTION 'Validation impossible : aucun colis n’est affecté à cette tournée';
  END IF;
  IF EXISTS (SELECT 1 FROM public.colis WHERE tournee_id = _tournee_id AND commande_id IS NULL) THEN
  ...
  SELECT col.commande_id, _tournee_id, COALESCE(v_type, 'direct'),
    COALESCE(max(col.ville_livraison), max(col.ville_destination)),
    max(col.livreur_nom), max(col.vehicule), 'preparee', count(*)::integer
  ...
  SELECT count(*) INTO v_suivis FROM public.livsuivi_commandes WHERE tournee_id = _tournee_id;
  IF v_suivis = 0 THEN
    RAISE EXCEPTION 'Validation impossible : aucun suivi de livraison n’a pu être créé';
  END IF;

  IF v_statut IN ('preparee', 'brouillon') THEN
    UPDATE public.tournees SET statut = 'en_cours', updated_at = now()
    WHERE tournee_id = _tournee_id;
    v_statut := 'en_cours';
  END IF;

  RETURN jsonb_build_object('tournee_id', _tournee_id, 'statut', v_statut, 'colis', v_colis, 'suivis', v_suivis);
END;
$$;
...
BEGIN
  IF NOT public.has_permission(auth.uid(), 'tournees.valider') THEN
    RAISE EXCEPTION 'Accès refusé : permission tournees.valider requise' USING ERRCODE = '42501';
  END IF;
  RETURN public.finaliser_tournee_interne(_tournee_id);
END;
$$;
...
REVOKE ALL ON FUNCTION public.finaliser_tournee_interne(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.finaliser_tournee_interne(uuid) FROM anon;
REVOKE ALL ON FUNCTION public.finaliser_tournee_interne(uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.finaliser_tournee_interne(uuid) TO service_role;
REVOKE ALL ON FUNCTION public.finaliser_tournee(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.finaliser_tournee(uuid) TO authenticated, service_role;
