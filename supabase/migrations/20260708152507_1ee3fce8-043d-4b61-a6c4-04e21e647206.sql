...
BEGIN
  IF current_setting('request.jwt.claims', true)::jsonb->>'role' = 'service_role' THEN
    RETURN OLD;
  END IF;
  IF current_setting('app.allow_paiement_delete', true) = 'on' THEN
    RETURN OLD;
  END IF;
...
BEGIN
  ...
  -- Autoriser la cascade de suppression des paiements liés pour cette transaction
  ...
  IF _livsuivi_ids IS NOT NULL THEN
    DELETE FROM public.livsuivi_historique WHERE commande_livsuivi_id = ANY(_livsuivi_ids);
