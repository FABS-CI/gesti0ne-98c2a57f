...
BEGIN
  ...
  -- Insertion en 'en_attente_validation' dans tous les cas.
  -- Si l'utilisateur peut valider, on chaîne valider_commande() ensuite,
  -- ce qui garantit le décrément stock + création atomique facture+BL.
  ...
  -- Auto-validation pour les utilisateurs habilités (chaînage atomique)
...
COMMENT ON FUNCTION public.creer_commande(jsonb) IS
  'Lot B — Crée la commande + proforma. Si l''utilisateur peut valider, chaîne valider_commande() qui gère atomiquement stock, facture et BL. Élimine les doublons.';
