...
DECLARE
  _result jsonb;
BEGIN
  IF NOT public.has_role_compat(auth.uid(), 'super_admin'::app_role) THEN
  ...
  SELECT jsonb_build_object(
    ...
    'policies', COALESCE((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        'sql', 'CREATE POLICY ' || quote_ident(policyname) || ' ON ' || quote_ident(schemaname) || '.' || quote_ident(tablename)
