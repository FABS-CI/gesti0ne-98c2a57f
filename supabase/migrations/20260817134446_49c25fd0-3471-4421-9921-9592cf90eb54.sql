...
BEGIN
  ...
  -- Vérification via le moteur RBAC v3
  IF NOT public.rbac3_can('logistique.supprimer', v_user) THEN
    RAISE EXCEPTION 'Accès refusé : permission logistique.supprimer requise pour supprimer une tournée'
      USING ERRCODE = '42501';
  END IF;
  ...
  v_is_super := public.rbac3_can('super_admin', v_user);

  IF NOT v_is_super THEN
    IF v_t.statut NOT IN ('preparee','en_cours','brouillon') THEN
      RAISE EXCEPTION 'Suppression interdite : la tournée est % — seul un super_administrateur peut la supprimer dans cet état.', v_t.statut
  ...
  v_summary := jsonb_build_object(
    ...
    'role_v3', CASE WHEN v_is_super THEN 'super_admin' ELSE 'user' END
  );
  ...
  VALUES (
    v_user,
    COALESCE(v_email, ''),
    CASE WHEN v_is_super THEN 'tournee_supprimee_super_admin' ELSE 'tournee_supprimee' END,
    'tournees',
    _tournee_id::text,
    to_jsonb(v_t),
    v_summary
