...
DROP FUNCTION IF EXISTS public.recalc_tournee_from_colis(date) CASCADE;
...
BEGIN
  IF _date IS NULL THEN
    RAISE LOG '[recalc_tournee] date NULL, skip'; RETURN 0;
  END IF;
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    v_u := public.recalc_tournee_from_colis(OLD.date_colisage);
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    IF NEW.date_colisage IS DISTINCT FROM OLD.date_colisage THEN
      v_u := public.recalc_tournee_from_colis(NEW.date_colisage);
