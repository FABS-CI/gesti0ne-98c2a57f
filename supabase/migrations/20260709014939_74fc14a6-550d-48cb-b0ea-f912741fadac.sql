...
-- A1 (suite) : corriger la référence colonne bl_id sur colisage_modifications_historique
...
BEGIN
    ...
    IF _ref IS NULL THEN RAISE EXCEPTION 'Commande introuvable'; END IF;
    ...
    IF _colis_ids IS NOT NULL THEN
      SELECT array_agg(DISTINCT date_colisage::date) INTO _dates
    ...
    END IF;
    -- FIX A1(suite): colisage_modifications_historique est indexé par bl_id, pas colis_id
    IF _bl_ids IS NOT NULL THEN
        DELETE FROM public.colisage_modifications_historique WHERE bl_id = ANY(_bl_ids);
    END IF;
    IF _colis_ids IS NOT NULL THEN
    ...
    VALUES ('DELETE'::audit_action, 'commandes', 'commandes', _commande_id::text, _ref, auth.uid(),
            jsonb_build_object(
              'motif', _motif, 'type', 'suppression_definitive',
...
-- A2 (revu) : rétablir annuler_colisage sans compensation stock.
-- Le stock est décrémenté à la validation de la commande (document_table='commandes'),
-- pas au colisage. Annuler_colisage ne doit donc pas y toucher — le stock est restitué
-- par supprimer_commande_definitif lorsque la commande elle-même est annulée.
...
BEGIN
  ...
  v_logistique_deja_pris := EXISTS (
    ...
    WHERE c.bl_id = _bl_id AND COALESCE(c.statut_logistique, 'prepare') <> 'prepare'
  );
  ...
  SELECT array_agg(DISTINCT date_colisage::date) INTO v_dates
  ...
  SET statut = 'annule', annule_at = now(), annule_par = v_user,
      annule_par_nom = COALESCE(auth.jwt()->'user_metadata'->>'nom_complet', auth.jwt()->>'email'),
      annulation_motif = _motif, updated_at = now()
  WHERE bl_id = _bl_id;
  ...
  VALUES (
    ...
    jsonb_build_object('motif', _motif, 'reference', v_bl.reference,
      ...
      'note', 'stock décrémenté par valider_commande ; restitution via supprimer_commande_definitif')
  );
END;
$function$;
