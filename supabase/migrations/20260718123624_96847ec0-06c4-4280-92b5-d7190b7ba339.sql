...
-- Distinguer retour physique (impact stock) vs avoir financier (pas d'impact stock)
ALTER TABLE public.retours
  ADD COLUMN IF NOT EXISTS type_retour text NOT NULL DEFAULT 'physique'
  CHECK (type_retour IN ('physique','avoir'));

-- ============================================================
-- creer_retour : ne touche au stock QUE si type_retour = 'physique'
-- ============================================================
...
BEGIN
  ...
  FOR v_l IN SELECT * FROM jsonb_array_elements(COALESCE(_payload->'lignes','[]'::jsonb)) LOOP
    ...
    -- Réintégration stock UNIQUEMENT si retour physique
...
-- ============================================================
-- annuler_retour : ne retire du stock que si retour était physique
-- ============================================================
...
BEGIN
  ...
  -- Retirer du stock ce qui avait été réintégré (physique uniquement)
  IF v_r.type_retour = 'physique' AND v_r.depot_id IS NOT NULL THEN
