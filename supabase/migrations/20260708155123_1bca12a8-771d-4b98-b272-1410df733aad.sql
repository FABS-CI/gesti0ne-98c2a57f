...
-- Lot 2 : cascade complète pour la suppression d'un colisage (BL)
DROP FUNCTION IF EXISTS public.supprimer_colisage(uuid, text);
...
BEGIN
  ...
  -- Snapshot des colis et des dates de colisage impactées
  SELECT array_agg(colis_id), array_agg(colis_id::text),
         array_agg(DISTINCT (date_colisage::date))
    INTO v_colis_ids, v_colis_ids_text, v_dates
    FROM public.colis WHERE bl_id = _bl_id;

  v_nb_colis := COALESCE(array_length(v_colis_ids, 1), 0);

  -- Suivi commande lié au BL
  ...
  -- Suppression des colis (cascade sur colis_lignes, colis_statut_historique)
  DELETE FROM public.colis WHERE bl_id = _bl_id;

  -- Marquer le BL comme colisage supprimé
  ...
  -- Recalcul des tournées impactées (par date de colisage)
  ...
  v_summary := jsonb_build_object(
    ...
    'motif', _motif,
    'colis_supprimes', v_nb_colis,
    'notifications_supprimees', v_nb_notifications,
    'envois_supprimes', v_nb_envois,
    'livsuivi_supprimes', v_nb_livsuivi,
    ...
    'tournees_recalculees', v_nb_tournees,
