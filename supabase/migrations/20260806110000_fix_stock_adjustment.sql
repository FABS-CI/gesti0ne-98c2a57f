...
BEGIN
  ...
  -- S'assurer que l'enregistrement existe (pour éviter le SELECT INTO v_ancienne qui reste NULL et bloque le calcul)
...
-- 3. Donner accès à l'utilisateur actuel à TOUS les dépôts s'il est au bureau
-- (C'est un correctif pour l'utilisateur spécifique du prompt si nécessaire)
-- On le fait de manière générique pour tout utilisateur ayant un rôle d'admin ou gestionnaire
