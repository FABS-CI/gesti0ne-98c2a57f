...
AS $$
  SELECT CASE
    ...
    WHEN n ILIKE '%bepc%'      THEN 170
    WHEN n ILIKE '%bac%'       THEN 180
    WHEN n ILIKE '%litt%' OR n ILIKE '%roman%' THEN 190
    ELSE 9999
  END
$$;

-- Le champ produits.niveau_ordre est GENERATED STORED : forcer un recalcul
-- pour appliquer immédiatement le nouveau mapping aux lignes existantes.
UPDATE public.produits SET niveau = niveau WHERE TRUE;