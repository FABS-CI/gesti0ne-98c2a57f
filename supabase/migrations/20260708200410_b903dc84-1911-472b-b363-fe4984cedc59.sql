DROP FUNCTION IF EXISTS public.supprimer_paiement_definitif(uuid, text);

CREATE FUNCTION public.supprimer_paiement_definitif(_paiement_id uuid, _motif text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT reference INTO v_ref FROM public.paiements WHERE paiement_id = _paiement_id;
  ...
  VALUES (auth.uid(), 'suppression_definitive', 'paiement', _paiement_id,
          jsonb_build_object('reference', v_ref, 'motif', _motif));
END;
$$;
